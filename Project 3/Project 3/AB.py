...
def include_next_pos(next_pos, is_white: bool, grid):
    if is_white:
        # blocked by its own piece
        return not grid[next_pos[0]][next_pos[1]].is_white
    else:
        return grid[next_pos[0]][next_pos[1]].is_white
...
class Board:
    def __init__(self, grid, given_pieces_num): 
        self.grid = grid
...
def studentAgent(gameboard):
    ...
    for pos, piece in gameboard.items():
        grid_pos = to_grid_coord(pos)
        piece_obj = Piece(piece[0], grid_pos, piece[1] == WHITE)
        grid[grid_pos[0]][grid_pos[1]] = piece_obj
...
def main():
    print(to_grid_coord(('a', 0)))
    print(to_chess_coord((0, 0)))
    board = setUpBoard()[2]
