...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    for i in range(len(own_pieces)):
        ...
        while not frontier.empty():
            ...
            # curr_node[0]: cost
            # curr_node[1]: grid coord position
            if goal_test(goals, curr_node[1])[0]:
                path_found = True
                first_goal_found = goal_test(goals, curr_node[1])[1]
                break

            piece = Piece(own_pieces[i][0], curr_node[1])
            ...
            for j in range(len(possible_moves)):
                ...
                if (neighbour != curr_node[1] and neighbour not in visited):
                    visited.add(neighbour)
                    parent[neighbour] = curr_node[1]
                    (r, c) = neighbour
                    curr_cost = curr_node[0] + grid[r][c]
                    frontier.put((curr_cost, neighbour))
    # backtrack
    if path_found:
